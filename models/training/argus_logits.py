...
def compute_kl(logits, gt, epsilon = 0.001):
    probs = torch.nn.functional.sigmoid(logits)
    d = 1/(1-2*epsilon)
    probls = probs/d + epsilon
    prob_dist = torch.stack([probs, 1-probs], dim=1)
    gt = gt/d + epsilon
    gt_dist = torch.stack([gt, 1-gt], dim=1)
    loss_fn = torch.nn.KLDivLoss(reduction='batchmean')
    loss_m = loss_fn(prob_dist.log(), gt_dist)
    return loss_m
...
def compute_js(logits, gt, epsilon = 0.001):
    ...
    gt_dist = torch.stack([gt, 1-gt], dim=1)
    loss_fn = JSD()
    loss_m = loss_fn(prob_dist, gt_dist)
    return loss_m
...
def train_cbm(args, model_class,train_loader,val_loader): 
    ...
    for e in range(args.n_epochs):
        ...
        for batch in tqdm(train_loader, desc=f'Epoch {e}'):
            ...
            eps = 1e-8
            probs = output.clamp(eps, 1 - eps)
            logits = torch.log(probs / (1 - probs))
            
            # Inject noise uniform
            std = 0.1
            low, high = -3 * std, 3 * std
            noise = torch.empty_like(logits).uniform_(low, high).to('cuda')

            # Inject noise gaussian
            #noise = torch.normal(mean=0, std=std, size=logits.shape).to('cuda')
            logits = logits + noise
...
def train(args):
    ...
    dataset_train = torch.load(f"/home/nicola.debole/projects/Argus/argus/annotated_datasets/{args.gp_model}.pt", weights_only=False)
    if hasattr(args, "gp_val") and args.gp_val != None:
        dataset_val = torch.load(f"/home/nicola.debole/projects/Argus/argus/annotated_datasets/{args.gp_val}.pt", weights_only=False)
    else:
        dataset_val = torch.load(f"/home/nicola.debole/projects/Argus/argus/annotated_datasets/{args.gp_model.replace('TRAIN','VAL')}.pt", weights_only=False)
    ...
    new_temp_args.dataset = f'{args.dataset}_argus'
