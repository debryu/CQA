...
# 
    ...
    for src_soft_concept in range(n_soft_concepts):
        ...
        for tgt_true_concept in range(n_true_concepts):
            ...
            # Train it
            estimator.fit(
                concept_soft_train_x[:, None],
                c_true[train_indexes, tgt_true_concept:(tgt_true_concept + 1)].ravel(),
                # **predictor_train_kwags,
            )

            # Compute the AUC of this classifier on the test data
            preds = estimator.predict(concept_soft_test_x[:, None])
            ...
            #     true_concepts = np.to_categorical(
            #         true_concepts,
...
if __name__ == "__main__":
    ...
    n_samples = 1000
    n_concepts = 39
    b = np.random.randint(0, 2, (n_samples, n_concepts))
    a = np.random.normal(0,10) + (b - 0.5)*100

    ois = oracle_impurity_score(a, b, predictor_model_fn=RandomForestClassifier)
    print(f'ois: {ois:.4f}')